package com.xiaopeng.speech.vui.task;

import com.xiaopeng.speech.vui.task.base.BaseTask;
import com.xiaopeng.speech.vui.utils.LogUtils;
import java.util.List;
/* loaded from: classes.dex */
public class TaskDispatcher {
    public String TAG = TaskDispatcher.class.getSimpleName();

    /* renamed from: com.xiaopeng.speech.vui.task.TaskDispatcher$1  reason: invalid class name */
    /* loaded from: classes.dex */
    public static /* synthetic */ class AnonymousClass1 {
        public static final /* synthetic */ int[] $SwitchMap$com$xiaopeng$speech$vui$task$TaskDispatcher$TaskType = new int[TaskType.values().length];

        static {
            try {
                $SwitchMap$com$xiaopeng$speech$vui$task$TaskDispatcher$TaskType[TaskType.ADD.ordinal()] = 1;
            } catch (NoSuchFieldError unused) {
            }
            try {
                $SwitchMap$com$xiaopeng$speech$vui$task$TaskDispatcher$TaskType[TaskType.BUILD.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            try {
                $SwitchMap$com$xiaopeng$speech$vui$task$TaskDispatcher$TaskType[TaskType.REMOVE.ordinal()] = 3;
            } catch (NoSuchFieldError unused3) {
            }
            try {
                $SwitchMap$com$xiaopeng$speech$vui$task$TaskDispatcher$TaskType[TaskType.UPDATE.ordinal()] = 4;
            } catch (NoSuchFieldError unused4) {
            }
            try {
                $SwitchMap$com$xiaopeng$speech$vui$task$TaskDispatcher$TaskType[TaskType.UPDATEATTRIBUTE.ordinal()] = 5;
            } catch (NoSuchFieldError unused5) {
            }
            try {
                $SwitchMap$com$xiaopeng$speech$vui$task$TaskDispatcher$TaskType[TaskType.UPDATERECYCLEVIEWITEM.ordinal()] = 6;
            } catch (NoSuchFieldError unused6) {
            }
        }
    }

    /* loaded from: classes.dex */
    public enum TaskType {
        BUILD,
        UPDATE,
        ADD,
        REMOVE,
        UPDATEATTRIBUTE,
        UPDATERECYCLEVIEWITEM
    }

    public void dispatchTask(List<TaskWrapper> list) {
        BaseTask buildSceneTask;
        if (list == null || list.isEmpty()) {
            return;
        }
        for (TaskWrapper taskWrapper : list) {
            int ordinal = taskWrapper.getTaskType().ordinal();
            if (ordinal == 0) {
                buildSceneTask = new BuildSceneTask(taskWrapper);
            } else if (ordinal == 1) {
                buildSceneTask = new UpdateSceneTask(taskWrapper);
            } else if (ordinal == 2) {
                buildSceneTask = new AddSceneTask(taskWrapper);
            } else if (ordinal == 3) {
                buildSceneTask = new RemoveTask(taskWrapper);
            } else if (ordinal == 4) {
                buildSceneTask = new UpdateSceneAttributeTask(taskWrapper);
            } else if (ordinal != 5) {
                buildSceneTask = new BuildSceneTask(taskWrapper);
            } else {
                buildSceneTask = new UpdateRecyclerItemTask(taskWrapper);
            }
            submitTask(buildSceneTask);
        }
    }

    public void removeTask(String str) {
        ThreadPoolProxyFactory.getThreadPool().removeTask(str);
    }

    public void submitTask(BaseTask baseTask) {
        if (baseTask == null || baseTask.wrapper == null) {
            return;
        }
        try {
            ThreadPoolProxyFactory.getThreadPool().execute(baseTask);
        } catch (Exception e) {
            String str = this.TAG;
            StringBuilder a = jg.a("submitTask e:");
            a.append(e.getMessage());
            LogUtils.e(str, a.toString());
            e.printStackTrace();
        }
    }
}
